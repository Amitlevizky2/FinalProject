...
export const Staff = (props) => {
    ...
    function createVolunteermailString(application, soldier, soldierAddres) {
        ...
        return `<h3><span dir=\\"rtl\\">פרטי הפניה:</span></h3><p><span dir=\\"rtl\\"><i>כותרת:</i><br />${application.event_title}<br /><i>תיאור:</i><br />${application.event_description}<br /></p><h3><span dir=\\"rtl\\">פרטי החייל:</span></h3><p><span dir=\\"rtl\\"><i>שם:</i><br />${soldier.display_name}<br /><i>מייל:</i><br />${soldier.email}<br /><i>מספר טלפון:</i><br />${soldier.phone}<br /><i>עיר:</i><br />${soldierAddres.city}<br /><i>שם הרחוב:</i><br />${soldierAddres.street_name}<br /><i>מספר:</i><br />${soldierAddres.street_number}</span></p>`
    }
