...
export const CreateTemplate = (api_key, msg_html) => {
    var urlParams = `entity=MessageTemplate&action=Create&json={"msg_title":"Application Details","msg_subject":"Confirmation", "msg_html":\"${msg_html}\","is_active":1}&api_key=${api_key}&key=${site_key}`;
    return client.post(`${postUrl}${urlParams}`)
};
